...
) const
{
	...
	while (itVertex != itVertexEnd)
	{
		*reinterpret_cast<CVector3*>(pVertexData) = *itVertex++;
		...
		{
			// Initialise vertex with no influences
	...
	if (pOutSubMesh->hasSkinningData)
	{
		// Offsets to weight data in a vertex (data is immediately after vertex coord)
		TUInt32 weightsOffset = sizeof(CVector3);
		int influencesOffset = weightsOffset + 4 * sizeof(TFloat32);
		...
		while (itBone != itBoneEnd)
		{
			...
			while (itBoneWeight != itBoneWeightEnd)
			{
				...
				TUInt8* pVert = pOutSubMesh->vertices + itBoneWeight->iVertexIndex * pOutSubMesh->vertexSize;
				TFloat32* pWeights = reinterpret_cast<TFloat32*>(pVert + weightsOffset);
				TUInt8* pInfluences = reinterpret_cast<TUInt8*>(pVert + influencesOffset);

				// Add influence of this bone to the vertex data
				AddBoneInfluence( itBone->iFrame, itBoneWeight->fWeight, pWeights, pInfluences );
				++itBoneWeight;
			}
			++itBone;
		}
		...
		for (TUInt32 vert = 0; vert < pOutSubMesh->numVertices; ++vert)
		{
			...
			TFloat32 sum = pWeights[0] + pWeights[1] + pWeights[2] + pWeights[3];
			if (sum == 0.0f)
			{
				// Vertex with no weights - reference root bone only (model is probably not skinned)
				pWeights[0] = 1.0f;
				pInfluences[0] = pOutSubMesh->node;
			}
			else
			{
				pWeights[0] /= sum;
