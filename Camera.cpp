...
CCamera::CCamera( DirectX::XMFLOAT3 position, DirectX::XMFLOAT3 rotation, float fov, float nearClip, float farClip )
{
	m_Position = position;
	m_Rotation = rotation;
...
void CCamera::UpdateMatrices()
{
	...
	DirectX::XMMATRIX matrixXRot, matrixYRot, matrixZRot, matrixTranslation;
	...
	//DirectX::XMMATRIX aMatrix = m_WorldMatrix;

	m_ViewMatrix = DirectX::XMMatrixInverse(NULL, m_WorldMatrix );
	...
	//DirectX::XMMATRIX aVM = m_ViewMatrix, aPM = m_ProjMatrix;
...
bool CCamera::PixelFromWorldPt( DirectX::XMFLOAT2* pixel, DirectX::XMFLOAT3 world)
{
	DirectX::XMFLOAT4 WorldPick = DirectX::XMFLOAT4( world.x, world.y, world.z, 1.0f);
	...
	WorldPick.w = DirectX::XMVectorGetW(temp);
