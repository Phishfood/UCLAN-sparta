...
{
	mModel = new CModel(position, DirectX::XMFLOAT3(0,0,0), 1.0f);
...
void CRenderObject::SetColour( DirectX::XMFLOAT3 colour )
{
	mr = colour.x;
	mg = colour.y;
	mb = colour.z;
...
void CRenderObject::Update(float frameTime)
{
	...
	DirectX::XMFLOAT3 currRotation = mModel->GetRotation();
