...
//--------------------------------------------------------------------------------------
#include "Defines.h"
#include "MultiModel.h"   // Declaration of this class

#include "CImportXFile.h"

ID3D10Device* CMultiModel::mpd3dDev;
...
CMultiModel::CMultiModel( D3DXVECTOR3 position, D3DXVECTOR3 rotation, float scale )
{
	m_Position = position;
	m_Rotation = rotation;
	SetScale( scale );
...
void CMultiModel::ReleaseResources()
{
	// Release resources
...
bool CMultiModel::Load( const string& fileName, ID3D10EffectTechnique* exampleTechnique, bool tangents /*= false*/ ) // The commented out bit is the default parameter (can't write it here, only in the declaration)
{
...
void CMultiModel::Control( float frameTime, EKeyCode turnUp, EKeyCode turnDown, EKeyCode turnLeft, EKeyCode turnRight,  
...
void CMultiModel::Render( ID3D10EffectTechnique* technique )
{
