...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
	...
	//Load in settings
	if (!mySettings->Load(L".\\Settings.ini"))
	{
		MessageBox(NULL, L"Error loading Settings.ini, unable to continue.", L"Error", MB_OK);
		return 1;
	}

	CScene myScene;
	myScene.SetSelf(&myScene);
	myScene.mySettings = mySettings;
